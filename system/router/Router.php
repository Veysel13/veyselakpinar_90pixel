...
class Router
{
    ...
    private static $namespaces  = [
        'controllers'   => 'Web\\',
        'service'       => 'WebService\\Controllers\\',
        'middlewares'   => 'System\\Middleware'
    ];
    ...
    public static function route($methods, $pattern, $callback)
    {
        ...
        if (is_callable($callback)) {
            ...
            if (self::$namespace) {
                ...
                if(stripos($callback, 'Service\\')!==false ){
                    $mycallback=explode('Service\\',$callback);
                    $closure = self::$namespaces['service'].$mycallback[1];
                }elseif(stripos($callback, '\\')!==false ){
                    $mycallback=explode('\\',$callback);
                    $closure = self::$namespaces['controllers'] .$mycallback[0].'\\'.'Controllers\\'.$mycallback[1];
                }else{
                    $closure = self::$namespaces['controllers'] .'Controllers'. '\\' . $callback;
                ...
                if ($domainCheck && $methodCheck) {
                    ...
                    if (array_key_exists('middlewares', $val)) {
                        ...
                        foreach ($val['middlewares'] as $midKey => $midVal) {
                            ...
                            if (class_exists($className)) {
                                ...
                                call_user_func_array([new $namespace, $method], array_values($params));
                            }
                        }
                    }
                    ...
                    if (is_callable($val['callback'])) {
                        ...
                        if (class_exists($className)) {
                            ...
                            call_user_func_array([new $className, $method], array_values($params));
                        } elseif (class_exists($namespace)) {
