...
class ThreeDPaymentInitRequest extends  IparaBase
{
    public $OrderId;
    public $Amount;
    public $UserId;
    ...
    public static function execute(ThreeDPaymentInitRequest $request, IparaSettings $settings)
    {
        ...
        $request->Token=IparaHelper::CreateToken($settings->PublicKey, $settings->HashString);
        return  $request->toHtmlString( $request, $settings);
    }

    public function toHtmlString(ThreeDPaymentInitRequest $request, IparaSettings $settings)
    {
        $builder = "";
