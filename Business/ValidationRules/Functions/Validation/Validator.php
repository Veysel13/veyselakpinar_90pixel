...
class Validator
{
        ...
        foreach ($rules AS $key => $value){
            if (is_array($value)) {
                foreach($value as $val) {
                    $this->validateType($data, $key, $val);
                }
            } else {
                $this->validateType($data, $key, $value);
            }
        }
     ...
     * @return  bool
     */
    protected function min($data, $length)
    {
     ...
     * @return  bool
     */
    protected function max($data, $length)
    {
        if(strlen(trim($data)) > $length) {
            return false;
        } else {
     ...
     * @return  bool
     */
    protected function len($data, $length)
    {
        if(strlen(trim($data)) == $length) {
            return true;
        } else {
    ...
    public function validateType($data , $key , $value) {
        $parts = explode("|", $value);
        ...
        for($i = 1; $i < count($parts); $i++){
            ...
            if(strpos($item, ':')) {
                ...
                if($filter == 'matches') {
                    if($this->matches($data[$key], $params) === false) {
                        $this->errors[] = $parts[0];
                    }
                } else {
